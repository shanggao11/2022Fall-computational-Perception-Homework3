...
C_IMGNET_MEAN = torch.tensor([[[[0.485, 0.456, 0.406]]]]).transpose(1, 3).to(device)
C_IMGNET_STD = torch.tensor([[[[0.229, 0.224, 0.225]]]]).transpose(1, 3).to(device)
...
def torch_gaussian_filter(img, sigma):
    ...
    return torch.tensor(ndimage.gaussian_filter(np_img, sigma=(0, 0, sigma, sigma)), requires_grad=True).to(device)
...
def add_noise(tensor, stddev):
    ...
    return tensor + dist.sample(tensor.shape).to(device)
